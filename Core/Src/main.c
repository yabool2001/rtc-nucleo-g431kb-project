...
/* USER CODE BEGIN PFP */
// Declare a function named __io_putchar that takes in an integer parameter named ch and returns an integer
int __io_putchar(int ch);

/*
 * The __io_putchar function is used to transmit a single character over UART.
 * It takes in an integer parameter ch which represents the character to be transmitted.
 * The function calls HAL_UART_Transmit to transmit the character over UART2.
 * The parameters for HAL_UART_Transmit are a pointer to UART2 handle (&huart2), a pointer to the character to be transmitted ((uint8_t *)&ch), the size of the data to be transmitted (1), and a timeout value (0xffff).
